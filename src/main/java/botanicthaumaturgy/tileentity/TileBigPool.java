 ...
 */
package botanicthaumaturgy.tileentity;
...
public class TileBigPool extends TilePool
        implements IManaPool, IDyablePool, IKeyLocked, ISparkAttachable, IThrottledPacket {

    public static final String tileEntityName = VersionInfo.ModID + ".bigPool";
    ...
    public boolean collideEntityItem(EntityItem item) {
        ...
        if (item.age > 100 && item.age < 130 || !catalystsRegistered) return false;
            ...
            if (recipe.matches(stack) && (!recipe.isAlchemy() || alchemy) && (!recipe.isConjuration() || conjuration)) {
                ...
                if (getCurrentMana() >= mana) {
                    ...
                    if (!worldObj.isRemote) {
                        ...
                        EntityItem outputItem = new EntityItem(
                                worldObj,
                                xCoord + 0.5,
                                yCoord + 1.5,
                                zCoord + 0.5,
    ...
    public void craftingFanciness() {
        ...
        for (int i = 0; i < 25; i++) {
            ...
            Botania.proxy.sparkleFX(
                    worldObj,
                    xCoord + 0.5 + Math.random() * 0.4 - 0.2,
                    yCoord + 1,
                    zCoord + 0.5 + Math.random() * 0.4 - 0.2,
                    red,
                    green,
                    blue,
                    (float) Math.random(),
        ...
        for (EntityItem item : items) {
            ...
            if (stack != null && stack.getItem() instanceof IManaItem) {
                ...
                if (outputting && mana.canReceiveManaFromPool(stack, this)
                    ...
                    int bellowCount = 0;
                    if (outputting) for (ForgeDirection dir : LibMisc.CARDINAL_DIRECTIONS) {
                    ...
                    if (outputting) {
                        if (canSpare) {
                            ...
                            int manaVal = Math.min(
                                    transfRate,
                                    Math.min(getCurrentMana(), mana.getMaxMana(stack) - mana.getMana(stack)));
                            if (!worldObj.isRemote) mana.addMana(stack, manaVal);
                            recieveMana(-manaVal);
                        }
                    } else {
                        if (canAccept) {
                            if (mana.getMana(stack) > 0 && !isFull()) didSomething = true;

                            int manaVal = Math
                                    .min(transfRate, Math.min(manaCap - getCurrentMana(), mana.getMana(stack)));
                            if (!worldObj.isRemote) mana.addMana(stack, -manaVal);
                            recieveMana(manaVal);
                        }
                    }
                    ...
                    if (didSomething) {
                        if (worldObj.isRemote && ConfigHandler.chargingAnimationEnabled
                                && worldObj.rand.nextInt(20) == 0) {
                            Vector3 itemVec = Vector3.fromTileEntity(this).add(0.5, 0.5 + Math.random() * 0.3, 0.5);
                            Vector3 tileVec = Vector3.fromTileEntity(this)
                                    .add(0.2 + Math.random() * 0.6, 0, 0.2 + Math.random() * 0.6);
                            LightningHandler.spawnLightningBolt(
                                    worldObj,
                                    outputting ? tileVec : itemVec,
                                    outputting ? itemVec : tileVec,
                                    80,
    ...
    public void readCustomNBT(NBTTagCompound cmp) {
        ...
        if (cmp.hasKey(TAG_MANA_CAP)) manaCap = cmp.getInteger(TAG_MANA_CAP);
        if (cmp.hasKey(TAG_CAN_ACCEPT)) canAccept = cmp.getBoolean(TAG_CAN_ACCEPT);
        if (cmp.hasKey(TAG_CAN_SPARE)) canSpare = cmp.getBoolean(TAG_CAN_SPARE);
        fragile = cmp.getBoolean(TAG_FRAGILE);

        if (cmp.hasKey(TAG_INPUT_KEY)) inputKey = cmp.getString(TAG_INPUT_KEY);
        if (cmp.hasKey(TAG_OUTPUT_KEY)) inputKey = cmp.getString(TAG_OUTPUT_KEY);

        if (cmp.hasKey(TAG_KNOWN_MANA)) knownMana = cmp.getInteger(TAG_KNOWN_MANA);
    }
    ...
    public void renderHUD(Minecraft mc, ScaledResolution res) {
        ItemStack pool = new ItemStack(Config.bigPool, 1, getBlockMetadata());
