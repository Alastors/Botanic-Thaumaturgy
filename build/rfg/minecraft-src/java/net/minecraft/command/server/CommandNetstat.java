    ...
    public void processCommand(ICommandSender sender, String[] args)
    {
        ...
        else
        {
            if (args.length > 0 && args[0].length() > 1)
            {
                ...
                {
                    sender.addChatMessage(new ChatComponentText(NetworkManager.field_152462_h.func_152477_e().toString()));
                }
                else if ("hottest-write".equals(args[0]))
                {
                    sender.addChatMessage(new ChatComponentText(NetworkManager.field_152462_h.func_152475_g().toString()));
                }
                else if ("most-read".equals(args[0]))
                {
                    sender.addChatMessage(new ChatComponentText(NetworkManager.field_152462_h.func_152467_f().toString()));
                }
                else if ("most-write".equals(args[0]))
                {
                    sender.addChatMessage(new ChatComponentText(NetworkManager.field_152462_h.func_152470_h().toString()));
                }
                else
                {
                    ...
                    {
                        sender.addChatMessage(new ChatComponentText("total-read-count" + String.valueOf(NetworkManager.field_152462_h.func_152472_c())));
                    }
                    else if ("write-count".equals(args[0]))
                    {
                        sender.addChatMessage(new ChatComponentText("total-write-count" + String.valueOf(NetworkManager.field_152462_h.func_152473_d())));
                    }
                    else
                    {
