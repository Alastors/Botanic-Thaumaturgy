 ...
 */
package botanicthaumaturgy.renderers.tileentityrenderers;
...
import botanicthaumaturgy.block.BlockBigPool;
import botanicthaumaturgy.renderers.models.ModelBigPool;
...
public class RenderTileBigPool extends TileEntitySpecialRenderer {
    ...
    private static final ModelBigPool model = new ModelBigPool();
    ...
    public void renderTileEntityAt(TileEntity tileentity, double d0, double d1, double d2, float f) {
        TileBigPool pool = (TileBigPool) tileentity;
        ...
        if (forceManaNumber > -1) mana = forceManaNumber;
        int cap = pool.manaCap;
        if (cap == -1) cap = TileBigPool.MAX_MANA;

        float waterLevel = (float) mana / (float) cap * 0.4F;
        if (forceMana) waterLevel = 0.4F;
        ...
        if (pool.getWorldObj() != null) {
            ...
            if (below instanceof IPoolOverlayProvider) {
                IIcon overlay = ((IPoolOverlayProvider) below)
                        .getIcon(pool.getWorldObj(), pool.xCoord, pool.yCoord - 1, pool.zCoord);
                if (overlay != null) {
                    GL11.glPushMatrix();
                    GL11.glEnable(GL11.GL_BLEND);
                    GL11.glBlendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA);
        ...
        if (waterLevel > 0) {
            ...
            renderIcon(0, 0, BlockBigPool.manaIcon, 16, 16, 240);
